...
class Hill(nn.Module):
    ...
    def __init__(self, lamb: float = 1.5, margin: float = 0.0, gamma: float = 0.0,  reduction: str = 'sum') -> None:
...
class AsymmetricLossAdaptiveWorking(nn.Module):
    def __init__(self, gamma_neg=0, gamma_pos=1, clip=0.0, eps=1e-8, disable_torch_grad_focal_loss=True, adaptive = True, gap_target = 0.1, gamma_step = 0.01):
            ...
            for i,(image,labels) in enumerate(dataloaders[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    #    targs = torch.where(preds > boundary.detach(), torch.tensor(1).to(preds), labels) # hard SPLC
                        targs = ((1-labels)*stepAtThreshold(preds, boundary.detach()) + labels).detach().clone() # soft SPLC
